...
因为BOSS侧维护用户与应用的关联关系，因此与BOSS侧相关的平台侧用户状态变化有如下几种：
1. 员工（包括企业管理员、企业普通员工）与企业解除关联关系
2. 企业管理员降级为普通员工
     
所以，平台侧在执行接触用户与企业关联、从管理员降级为普通用户时，均需要向BOSS查询被操作对象在BOSS侧是否关联应用，BOSS进行根据用户在BOSS侧的状态进行回应，有如下场景：

| Ret | Code | Msg |
|:--- |:--- |:--- |
| 0 | 0 | BOSS侧无此用户，可以删除/降低 |
| 0 | 1 | BOSS侧有此用户，用户是管理员，用户未关联应用，可以删除/降级 |
| 0 | 2 | BOSS侧有此用户，用户是普通用户，用户未关联应用，可以删除 | 
| 1 | 3 | BOSS侧有此用户，用户是管理员，用户关联已购应用（把关联已购的产品名称展示出来），不可删除/降级 | 
| 1 | 4 | BOSS侧有此用户，用户是管理员，用户关联被授权应用（把关联被授权的产品名称展示出来），不可删除 |
| 1 | 5 | BOSS侧有此用户，用户是普通用户，用户关联被授权应用（把关联被授权的产品名称展示出来），不可删除 |